...
import { Pencil, CheckCircle2, Circle, Clock, ArrowDown, ArrowUp } from 'lucide-react';
...
export function ReminderRow({ reminder, onEdit, onToggleStatus }: ReminderRowProps) {
    ...
    return (
        <div
            className={`group  flex items-center justify-between p-4 transition-all hover:bg-surface-800/50 ${isPaid ? 'opacity-50' : ''
                }`}
        >
            <div className="flex items-center gap-4 min-w-0" onClick={onToggleStatus}>
                <button
                    className={`flex-shrink-0 transition-colors ${isPaid ? 'text-emerald-500' : 'text-gray-500 hover:text-indigo-400'
                        }`}
                >
            ...
            <div className="flex flex-col items-end gap-1 flex-shrink-0 ml-4">
                <p className={`font-semibold ${isPaid ? 'text-gray-500' : isReceivable ? 'text-emerald-400' : 'text-gray-200'
                    ...
                    {isReceivable ? '+' : '-'}{formatBRL(reminder.amount)}
                </p>

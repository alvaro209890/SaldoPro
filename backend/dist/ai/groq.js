...
exports.resolveBestCategoryId = resolveBestCategoryId;
...
function findCategoryByCanonicalName(categories, canonical) {
...
function findMentionedCategory(categories, text) {
 ...
 */
function buildCategoryKeywordGuide(categories) {
    ...
    const lines = ['GUIA DIRETO DE CATEGORIAS DO USUARIO (nome -> categoryId):'];
 ...
 */
function resolveBestCategoryId(aiCategoryId, categories, transactionType, description, userMessage) {
    if (categories.length === 0)
        return null;
...
${settings.budget > 0 ? `Orcamento mensal definido: ${formatCurrency(settings.budget, settings.currency)}` : 'Sem orcamento mensal definido.'}`;
        ...
        const txList = recentTransactions
            .slice(0, PROMPT_TX_LIMIT)
            .map((t) => `- ID: "${t.id}", Codigo: ${toFriendlyTransactionCode(t.id)}, Data: ${t.date}, Desc: "${t.description}", Valor: ${t.amount}, Tipo: ${t.type}, CatID: ${t.category}`)
            .join('\n');
...
function fallbackGeneratedGoalDeadline(today, priority, index) {
    const baseDays = priority === 'high'
        ? 90
        : priority === 'low'
            ? 365
            : 180;
    const spreadDays = priority === 'high'
        ? 30
        : priority === 'low'
            ? 60
            : 45;
...
function normalizeGeneratedGoalDeadline(rawDeadline, today, priority, index) {
    if (!rawDeadline) {
    ...
    for (const model of GROQ_MODEL_CHAIN.filter(m => !m.vision)) {
        try {
            ...
            const goals = parsed.goals
                ...
                .slice(0, 5)
                .map((g, index) => {
                const priority = g.priority === 'high' ? 'high' : g.priority === 'low' ? 'low' : 'medium';
            ...
            if (goals.length > 0) {
                logger_1.logger.info('generateFinancialGoals: success', {
                    model: model.id,
